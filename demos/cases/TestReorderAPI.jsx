...
export default function TestReorderAPI() {
  ...
  const createTestData = async () => {
    ...
    try {
      ...
      const projectPath = config.type === 'project' ? config.projectId : config.groupId;
      ...
      const issue1Response = await gitlabRestRequest(
        ...
        },
        {
          ...
          body: JSON.stringify({
            ...
            description: 'This is a test issue for API validation. Safe to delete.',
          }),
        }
      );

      addLog(`✅ Created Issue 1: #${issue1Response.iid} (ID: ${issue1Response.id})`, 'success');
      ...
      addLog(`✅ Created Issue 2: #${issue2Response.iid} (ID: ${issue2Response.id})`, 'success');
  ...
  const testIssueReorder = async () => {
    ...
    try {
      ...
      addLog(`Moving Issue #${testData.issue2Iid} after Issue ID ${testData.issue1Id}`, 'info');
      ...
      addLog('🎉 REST API is working! Safe to use for Issue reordering.', 'success');
    } catch (error) {
  ...
  const createTestTasks = async () => {
    ...
    try {
      ...
      const task1Result = await provider.graphqlClient.mutate(task1Mutation, {});
      ...
      if (task1Result.workItemCreate.errors?.length > 0) {
        throw new Error(`Failed to create Task 1: ${task1Result.workItemCreate.errors.join(', ')}`);
      }
      ...
      const task2Result = await provider.graphqlClient.mutate(task2Mutation, {});
      ...
      if (task2Result.workItemCreate.errors?.length > 0) {
        throw new Error(`Failed to create Task 2: ${task2Result.workItemCreate.errors.join(', ')}`);
      }
  ...
  const introspectHierarchyWidget = async () => {
    setIsLoading(true);
    addLog('🔍 Introspecting GraphQL schema for WorkItemWidgetHierarchyUpdateInput...', 'info');
    ...
    try {
      ...
      if (result.__type) {
        ...
        addLog('Available fields in WorkItemWidgetHierarchyUpdateInput:', 'info');
  ...
  const testTaskReorder = async () => {
    ...
    try {
      ...
      addLog(`Moving Task ${testData.task2GlobalId} AFTER ${testData.task1GlobalId}`, 'info');
      addLog('Using adjacentWorkItemId + relativePosition (correct API for GitLab 18.2.6)', 'info');
      ...
      addLog(JSON.stringify(result.workItemUpdate.workItem, null, 2), 'success');
      addLog('🎉 GraphQL hierarchyWidget with adjacentWorkItemId is working!', 'success');
  ...
  const runFullTest = async () => {
    ...
    addLog('This will test Issue and Task reordering with verification', 'info');
    ...
    try {
      ...
      addLog(`🧪 GOAL: Understand the true meaning of move_after_id parameter`, 'info');
      addLog(``, 'info');
      addLog(`Initial state: Issue #${newTestData.issue1Iid} and Issue #${newTestData.issue2Iid} created`, 'info');
      addLog(``, 'info');
      addLog(`📝 What we want to achieve:`, 'info');
      addLog(`   Visual order in list (top to bottom): Issue #${newTestData.issue1Iid} → Issue #${newTestData.issue2Iid}`, 'info');
      addLog(`   (Issue #${newTestData.issue1Iid} should appear ABOVE/BEFORE Issue #${newTestData.issue2Iid})`, 'info');
      addLog(``, 'info');
      addLog(`🔬 API Call: PUT /issues/${newTestData.issue2Iid}/reorder`, 'info');
      addLog(`   Parameter: move_after_id=${newTestData.issue1Id}`, 'info');
      addLog(``, 'info');
      addLog(`❓ Question: Does "move_after_id" mean:`, 'info');
      addLog(`   A) Issue #${newTestData.issue2Iid} will appear AFTER (below) Issue #${newTestData.issue1Iid} ✓ (expected)`, 'info');
      addLog(`   B) Issue #${newTestData.issue2Iid} will appear BEFORE (above) Issue #${newTestData.issue1Iid} ✗ (inverted)`, 'info');
      ...
      if (testIssues.length === 2) {
        const issue1Index = testIssues.findIndex(i => i.iid === newTestData.issue1Iid);
        const issue2Index = testIssues.findIndex(i => i.iid === newTestData.issue2Iid);
        ...
        if (issue1Index < issue2Index) {
          ...
          addLog(`   → Issue #${newTestData.issue2Iid} is AFTER (below) Issue #${newTestData.issue1Iid}`, 'success');
          addLog(`   → This is Answer A: "after" means below/later in the list`, 'success');
        } else {
          // Issue 2 is at index 0 (top), Issue 1 is at index 1 (bottom)
          addLog(`   ❌ INVERTED: move_after_id has opposite behavior!`, 'error');
          addLog(`   → Issue #${newTestData.issue2Iid} is BEFORE (above) Issue #${newTestData.issue1Iid}`, 'error');
          addLog(`   → This is Answer B: "after" actually means above/earlier in the list`, 'error');
          addLog(`   → We need to use move_before_id to achieve "move after" behavior`, 'error');
        }
      } else {
        addLog(`⚠️ Expected 2 test issues but found ${testIssues.length}`, 'error');
      }
      ...
      addLog(`🧪 GOAL: Move Issue #${newTestData.issue2Iid} to FIRST position`, 'info');
      addLog(``, 'info');
      addLog(`Current order: Issue #${newTestData.issue2Iid} is at 1st, Issue #${newTestData.issue1Iid} is at 2nd`, 'info');
      addLog(`Target order: Issue #${newTestData.issue1Iid} at 1st, Issue #${newTestData.issue2Iid} at 2nd`, 'info');
      addLog(``, 'info');
      addLog(`🔬 API Call: PUT /issues/${newTestData.issue1Iid}/reorder`, 'info');
      addLog(`   Parameter: move_after_id=${newTestData.issue2Id}`, 'info');
      addLog(`   (Due to inverted API, this should move Issue #${newTestData.issue1Iid} BEFORE Issue #${newTestData.issue2Iid})`, 'info');
      ...
      const testIssues2 = allIssues2.filter(i =>
        i.iid === newTestData.issue1Iid || i.iid === newTestData.issue2Iid
      );
      ...
      if (testIssues2.length === 2) {
        const issue1Index2 = testIssues2.findIndex(i => i.iid === newTestData.issue1Iid);
        const issue2Index2 = testIssues2.findIndex(i => i.iid === newTestData.issue2Iid);
        ...
        if (issue1Index2 === 0 && issue2Index2 === 1) {
          ...
          addLog(`   → Issue #${newTestData.issue1Iid} is now at FIRST position`, 'success');
          addLog(`   → Using move_after_id with inverted API successfully moves to first`, 'success');
        } else {
          addLog(`   ❌ FAILED: "Move to First" did not work`, 'error');
          addLog(`   → Expected Issue #${newTestData.issue1Iid} at index 0, got index ${issue1Index2}`, 'error');
          addLog(`   → Expected Issue #${newTestData.issue2Iid} at index 1, got index ${issue2Index2}`, 'error');
        }
      }
      ...
      addLog(`✅ Created Task 1: ${task1GlobalId} (IID: ${task1.id})`, 'success');
      ...
      addLog(`✅ Created Task 2: ${task2GlobalId} (IID: ${task2.id})`, 'success');
      ...
      const taskReorderResult = await provider.graphqlClient.mutate(taskReorderMutation, {});
      ...
      const workItems = config.type === 'group'
      ...
      if (workItems.length > 0) {
        ...
        addLog(`Current task order under parent Issue #${newTestData.parentIssueIid}:`, 'info');
        ...
        if (task1Index >= 0 && task2Index >= 0) {
          if (task1Index < task2Index) {
            addLog('✅ Task order is CORRECT: Task 1 comes before Task 2', 'success');
          } else {
            addLog('❌ Task order is WRONG: Task 2 should come after Task 1', 'error');
          }
        }
      }

      addLog('\n✅ Full test suite completed!', 'success');
      addLog('You can now cleanup the test data using the Cleanup button', 'info');
  ...
  const verifyIssueOrder = async () => {
    ...
    try {
      ...
      testIssues.forEach((issue, index) => {
        addLog(`  ${index + 1}. Issue #${issue.iid}: ${issue.title} (relativePosition: ${issue.relativePosition})`, 'info');
      });

      // Check if order is correct (Issue 1 should come before Issue 2)
      if (testIssues.length === 2) {
        const issue1Index = testIssues.findIndex(i => i.iid === testData.issue1Iid);
        const issue2Index = testIssues.findIndex(i => i.iid === testData.issue2Iid);

        if (issue1Index < issue2Index) {
          addLog('✅ Issue order is CORRECT: Issue 1 comes before Issue 2', 'success');
        } else {
          addLog('❌ Issue order is WRONG: Issue 2 should come after Issue 1', 'error');
        }
      }
  ...
  const verifyTaskOrder = async () => {
    ...
    try {
      ...
      if (workItems.length > 0) {
        ...
        addLog(`Current task order under parent Issue #${testData.parentIssueIid}:`, 'info');
        children.forEach((child, index) => {
          addLog(`  ${index + 1}. Task ${child.id}: ${child.title}`, 'info');
        });

        // Check if order is correct (Task 1 should come before Task 2)
        const task1Index = children.findIndex(c => c.id === testData.task1GlobalId);
        const task2Index = children.findIndex(c => c.id === testData.task2GlobalId);
  ...
  const cleanup = async () => {
    ...
    try {
      ...
      if (testData.parentIssueIid) {
        ...
        addLog(`✅ Deleted Parent Issue #${testData.parentIssueIid}`, 'success');
      }
    ...
    return (
      <div style={{ padding: '20px' }}>
        ...
        <p style={{ color: '#e74c3c' }}>
          ⚠️ GitLab is not configured. Please configure GitLab settings first in the GitLab Gantt page.
        </p>
      </div>
    );
  }
    ...
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <p style={{ margin: '5px 0', color: '#856404' }}>
          This test validates whether GitLab's native reorder APIs work in your version:
        </p>
        <ul style={{ margin: '5px 0', color: '#856404' }}>
          <li><strong>REST API</strong>: <code>PUT /api/v4/projects/:id/issues/:iid/reorder</code> for Issue ordering</li>
          <li><strong>GraphQL API</strong>: <code>workItemUpdate</code> with <code>hierarchyWidget.moveAfterId</code> for Task ordering</li>
        </ul>
        <p style={{ margin: '5px 0', color: '#856404' }}>
          If both work, we can replace description metadata with native GitLab positioning.
        </p>
      </div>
      ...
      <div style={{
        ...
        <h3>Configuration</h3>
        <p><strong>GitLab URL:</strong> {config.gitlabUrl}</p>
        <p><strong>Project/Group:</strong> {config.projectId || config.groupId}</p>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '4px',
              cursor: isLoading || !testData.issue1GlobalId ? 'not-allowed' : 'pointer',
              fontSize: '14px',
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '4px',
              cursor: isLoading || !testData.parentIssueGlobalId ? 'not-allowed' : 'pointer',
              fontSize: '14px',
            }}
          >
            3. Create Test Tasks
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '4px',
              cursor: isLoading || !testData.task1GlobalId ? 'not-allowed' : 'pointer',
              fontSize: '14px',
            }}
          >
            4. Test Task Reorder
      ...
      <div style={{
        ...
        {logs.length === 0 ? (
          <p style={{ color: '#95a5a6' }}>No logs yet. Click a button to start testing.</p>
        ) : (
          logs.map((log, index) => (
            <div
              key={index}
